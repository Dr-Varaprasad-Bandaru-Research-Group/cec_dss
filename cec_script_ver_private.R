...
load_private <- function(){
  log("loading ownership data")
  own.prj <<- load_preproc_raster("own_proj.tif")
  cdl_without_own <<- mask(cdl.prj.mask,own.prj,inverse=T) # CDL without ownership
  cdl_without_own[is.na(cdl_without_own[])]<-0
  writeRaster(cdl_without_own,"cdl_without_own.tif",overwrite=T)
  fl.ele.private <<- mask(fl.ele,cdl_without_own,maskvalue=0)
  ...
  fl_ele_stack = stack(fl.ele,fl.ele.private)
  plot(fl_ele_stack)
  #plot(own_stack)
