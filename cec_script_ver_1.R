...
mask_own_wild_in <- function(){
  fl.ele.wild.in <<- load_preproc_raster("fl_ele_wild_in.tif")
  fl.ele.wild.cdl.own.in <<- load_preproc_raster("fl_ele_wild_cdl_own_in.tif")
  
}

mask_own_wild_ex <- function(){
  log("Getting mask for excluding ownership and wilderness")
  
  # wilderness + ele
  fl.ele.wild.in <<- load_preproc_raster("fl_ele_wild_in.tif")
  
  # ele and cdl, with wilderness & ownership excluded
  fl.ele.wild.cdl.own <<- load_preproc_raster("fl_ele_wild_cdl_own_ex.tif")
  
  # ele and cdl with wilderness & ownership included 
  # fl.ele.wild.cdl.own.in <<- load_preproc_raster("fl_ele_wild_cdl_own_in.tif")
  ...
  fl.ele.wild.cdl.own.masked <- load_preproc_raster("fl_ele_wild_cdl_own_masked_ex.tif")
  
  # Convert to data frame
  df.ele <<- rasterToPoints(fl.ele.wild.cdl.own.masked, spatial = T) #elevation data, acts as final mask
  #writeOGR(df.ele, "F3_Data_points_v3", "Elevation_ex_wild_own_ver_1", driver="ESRI Shapefile")
...
Visualization <- function(county_name){
  ...
  plot_file <- paste("/plot/",county_name,"_ex_wild_own.tif",sep="")
...
county_names <<- c("Alpine","Amador","Butte","Calaveras","El Dorado","Fresno","Inyo","Kern","Madera","Mariposa",
                   "Mono","Nevada","Placer","Tulare","Tuolumne","Yuba")
